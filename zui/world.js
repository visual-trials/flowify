...
//        sliceConnectionsByIdentifier[worldConnection.identifier] = worldSliceConnection // TODO: should we use a differnt (non-world) identifier for sliceConnections?
    ...
    my.placeSliceContainerInAllowedSize = function (world, currentSliceContainer, allowedSize) {

        // TODO: Determine the maxSize (maxWidth and maxHeight) of an element using its children (or a fixed value if it doesn't have children).
        //       If the allowedSize becomes larger than the maxSize, limit the size of the currentSliceContainer, and propagate this back to the caller of this function.
        //       That caller should adjust how it places it's children, so that it maximizes the information on the size available.
        // TODO: For example: call getSizeOfElementInParent for each child in the Container! (assuming unlimited allowedSize)
        // TODO: If a map has no children, use the minimum height and the parents width?? --> BUT what if parent's width is unlimited?? return null for width?? (so it will be set by parent)
        // TODO: aspect ratio: this should be dependent on the room in the parentContainer and the other children... (and the children of THIS container)

        // getChildConnectionGraph/Trees
        //    determine which child is connected to what other childs
        //    connections are directed (from-to) and these graphs don't have to be strict trees (they can be interconnected/ be a network), but should be accessible from a (set of) tree-root(s)
        //    Note: if a child is connected to a grand-child (so a child of a child), then it is considered to be connected to the child (for the purpose of the fit-algo)
        //    Note: circular connections are is not permitted

        // size = getMaxContainerSize (width and height)
        //     foreach child:
        //         getMaxConainerSize (recursive-call)
        // position all childs (with its max Size) according to some fit-algorithm, with no boundaries, but which wants to make the smallest fit (area wise, possibly preferring some aspect ratio)
        // the fit-algorithm returns a maxSize, we return this

        // createZoomPyramid
        // - start with max zoom of all elements (so fully pannable at 100%)
        //     - from the maximum zoom, zoom back n %, using current density/strength per element
        //     - re-fit the elements into their containers given their new size, remove when nessesary (goal: maximize density/strength).
        //     - recalculate the density/strength per element
        //     - record size/aspect ratio and (relative/normalized) positions of all elements at 'each' zoomlevel
        //     - repeat until all elements fit into the screen

        // placeSliceContainerInAllowedSize OR placeChildrenInAllowedSize
        // - for each child
        //      - call getSizeAndPositionAtZoom (using zoomPyramid)
        //      - recursively call placeSliceContainerInAllowedSize or placeChildrenInAllowedSize
        //      - place child-containers accordingly
        // - place currentSliceContainer (set its position and size)
        ...
        var currentSize = currentSliceContainer.size
        var currentNormalizedSize = currentSliceContainer.worldContainer.normalizedSize
        var minSize = {
            width: containerProperties.minWidth,
            height: containerProperties.minHeight
        }
        var maxSize = {
            width: containerProperties.maxWidth,
            height: containerProperties.maxHeight
        }

        // FIXME: there is a function called getContainerHeaderHeight, but the below doesn't use the container.size.height!
        var headerHeight = 0
        if (containerProperties.showHeader) {
            if (containerProperties.hasFixedHeaderHeight) {
                ...
                // TODO: currentSize.height might not have been set!
                // TODO: this is only possible if currentNormalizedSize.height is available! Shouldn't this be allowedSize.height instead?
                headerHeight = currentSize.height * (containerProperties.normalizedHeaderHeight / currentNormalizedSize.height)
            }
        }
        ...
        var keepAspectRatio = containerProperties.keepAspectRatio
        var aspectRatioAtMaxSize = null
        if (keepAspectRatio) {
            if (containerProperties.aspectRatioAtMaxSize != null) {
                aspectRatioAtMaxSize = containerProperties.aspectRatioAtMaxSize
            }
            else {
                // TODO: what if currentSize.height is 0?
                // TODO: this is only possible if currentNormalizedSize.height is available!
                aspectRatioAtMaxSize = currentNormalizedSize.width / currentNormalizedSize.height
            }
        }

        var childrenFitFunction = containerProperties.childrenFitFunction

        var addGuides = false
        if (my.selectedSliceContainer != null && my.selectedSliceContainer.worldContainer.identifier == currentSliceContainer.worldContainer.identifier) {
            addGuides = true
        }

        if (addGuides) {
// FIXME: memory leak!            my.addGuidePoint(world, currentSliceContainer, 0, currentSliceContainer.size.height)
        }


        // Start of actual placing of the container
        {

            if (keepAspectRatio) {
                // should we keep the aspect ratio? or should it transform from the aspect ratio in minimum size towards the aspect ratio in maximum size?
                if ((allowedSize.width / allowedSize.height) > aspectRatioAtMaxSize) { // parent is relatively wider than we would be if maxed out
                    // Use the parent's height as the limit
                    currentSize.height = allowedSize.height
                    currentSize.width = currentSize.height * aspectRatioAtMaxSize
                }
                else {
                    // Use the parent's width as the limit
                    currentSize.width = allowedSize.width
                    currentSize.height = currentSize.width / aspectRatioAtMaxSize
                }
            }
            else {
                currentSize.width = allowedSize.width
            ...
            if (maxSize.width != null && currentSize.width > maxSize.width) {
                currentSize.width = maxSize.width
            }
            if (maxSize.height != null && currentSize.height > maxSize.height) {
                currentSize.height = maxSize.height
            }

            var innerSize = {
                width: currentSize.width - paddingLeft - paddingRight,
                height: currentSize.height - headerHeight - paddingTop - paddingBottom
            }

            var paddingFromLeftBottom = {
                x: paddingLeft,
                y: paddingBottom
//                    y: headerHeight + paddingTop
            }

            var fromParentPositionToCenterOfParent = my.getCenterPositionBasedOnPointsTo({ x: 0, y:0 }, innerSize, positionPointsTo)

            var fromParentPositionToLeftBottomOfParent = {}
            fromParentPositionToLeftBottomOfParent.x = fromParentPositionToCenterOfParent.x - innerSize.width / 2
            fromParentPositionToLeftBottomOfParent.y = fromParentPositionToCenterOfParent.y - innerSize.height / 2


            if (currentSliceContainer.hasOwnProperty('sliceChildren')) {

                if (childrenFitFunction == 'verticalTopToBottom') {

                    var verticalMarginInBetween = 20

                    var childAllowedSize = {}
                    childAllowedSize.width = innerSize.width
                    var heightUsedForInBetweenMargins = (currentSliceContainer.sliceChildren.length - 1) * verticalMarginInBetween

//                    var currentTopPosition = paddingFromLeftBottom.y
                    var currentTopPosition = paddingFromLeftBottom.y + innerSize.height

                    var loopIndex
                    for (loopIndex = 0; loopIndex < currentSliceContainer.sliceChildren.length; loopIndex++) {
                        var childSliceContainer = currentSliceContainer.sliceChildren[loopIndex]

                        // TODO: maybe do this check inside placeSliceContainerInAllowedSize?
                        // TODO: maybe minimum size is determined by the containerType, not the specific container.

                        childAllowedSize.height = (innerSize.height - heightUsedForInBetweenMargins) / currentSliceContainer.sliceChildren.length

                        // TODO: compare the actual size of the child AFTER this call, and adjust the room available for other childs accordingly!
                        my.placeSliceContainerInAllowedSize(world, childSliceContainer, childAllowedSize)

                        // FIXME: guessed here that we should compare with worldContainer.containerProperties.minWidth/Height. Correct?
                        if (childAllowedSize.width >= childSliceContainer.worldContainer.containerProperties.minWidth && childAllowedSize.height >= childSliceContainer.worldContainer.containerProperties.minHeight) {
                        // if (true) {
                            childSliceContainer.isVisible = true

                            // FIXME: add fromParentPositionToLeftBottomOfParent.x/y + fromLeftBottomOfParentToBaseChildPosition.x/y
                            childSliceContainer.relativePosition.x = paddingFromLeftBottom.x
                            // TODO: should we use the room made by invisible childs?

//                            childContainer.relativePosition.y = currentTopPosition
                            childSliceContainer.relativePosition.y = currentTopPosition - childSliceContainer.size.height

                            currentTopPosition -= (verticalMarginInBetween + childSliceContainer.size.height)
                            //childContainer.relativePosition.y = loopIndex * childAllowedSize.height + loopIndex * verticalMarginInBetween
                        }
                        else {
                            childSliceContainer.isVisible = false
                        }
                        ...
                        if (addGuides) {
// FIXME: memory leak!                            my.addGuidePoint(world, childSliceContainer, 0, childSliceContainer.size.height)
// FIXME: memory leak!                            my.addGuidePoint(world, childSliceContainer, childSliceContainer.size.width, childSliceContainer.size.height)
                        }
                    }

                }
                else if (childrenFitFunction == 'horizontalFromSidesToCenter') {

                    // TODO: get this from containerTypeProperty
                    var horizontalMarginInBetween = 20

                    var widthUsedForInBetweenMargins = (currentSliceContainer.sliceChildren.length - 1) * horizontalMarginInBetween
                    ...
                    for (loopIndex = 0; loopIndex < currentSliceContainer.sliceChildren.length; loopIndex++) {
                        ...
                        var childAllowedSize = {}
                        childAllowedSize.width = (innerSize.width - widthUsedForInBetweenMargins) / currentSliceContainer.sliceChildren.length
                        childAllowedSize.height = innerSize.height
                        ...
                        if (childAllowedSize.width >= childSliceContainer.worldContainer.containerProperties.minWidth && childAllowedSize.height >= childSliceContainer.worldContainer.containerProperties.minHeight) {
                            ...
                            // FIXME: add fromParentPositionToLeftBottomOfParent.x/y + fromLeftBottomOfParentToBaseChildPosition.x/y
                            childSliceContainer.relativePosition.x = paddingFromLeftBottom.x + (loopIndex * (horizontalMarginInBetween + ((innerSize.width - widthUsedForInBetweenMargins) / currentSliceContainer.sliceChildren.length)))
                            childSliceContainer.relativePosition.y = paddingFromLeftBottom.y
                        }
                        else {
                            childSliceContainer.isVisible = false
                        }
...
// FIXME: memory leak!                            my.addGuidePoint(world, childSliceContainer, 0, childSliceContainer.size.height)
                ...
                else if (childrenFitFunction == 'normalizedRelativePositioning') {

                    var opacityChildren = 1.0

                    var loopIndex
                    for (loopIndex = 0; loopIndex < currentSliceContainer.sliceChildren.length; loopIndex++) {
                        var childSliceContainer = currentSliceContainer.sliceChildren[loopIndex]

                        var childContainerProperties = childSliceContainer.worldContainer.containerProperties
                        var normalizedRelativePositionPointsTo = childContainerProperties.normalizedRelativePositionPointsTo

                        // Note: this function assumes the relativePosition (first argument) to be the left-bottom of the rectangle
                        var fromLeftBottomOfParentToBaseChildPosition = my.getRelativePositionInRectangleBasedOnPointsTo(innerSize, normalizedRelativePositionPointsTo)

                        var childAllowedSize = {}

                        var childOpacity = currentSliceContainer.opacity // TODO: should we reset opacity each time or pass it from parent through child like here?
                        if (containerProperties.fadeOutChildrenBasedOnPercentageOfNormalSize) {
                            var childrenAtFullOpacityThreshold = containerProperties.childrenAtFullOpacityThreshold
                            var childrenAtZeroOpacityThreshold = containerProperties.childrenAtZeroOpacityThreshold

                            var percentageOfNormalSize = currentSliceContainer.size.height / currentSliceContainer.worldContainer.normalizedSize.height
                            if (percentageOfNormalSize < childrenAtZeroOpacityThreshold) {
                                childOpacity = 0.0
                            }
                            else if (percentageOfNormalSize > childrenAtFullOpacityThreshold) {
                                childOpacity = 1.0
                            }
                            else {
                                childOpacity = (percentageOfNormalSize - childrenAtZeroOpacityThreshold) / (childrenAtFullOpacityThreshold - childrenAtZeroOpacityThreshold)
                                // ZUI.log('h:' + currentSliceContainer.size.height + ' nh:' + currentSliceContainer.worldContainer.normalizedSize.height)
                            }

                            // FIXME: maybe we want the sum of these two be higher or lower than 1.0?
                            currentSliceContainer.containerTextOpacity = 1.0 - childOpacity
                        }


                        childAllowedSize.height = (childSliceContainer.worldContainer.normalizedSize.height / currentNormalizedSize.height) * innerSize.height
                        // FIXME: this is a HACK to get the width of the label! This should also be dependent on the relative font vs height size!
                        if (childSliceContainer.worldContainer.normalizedSize.width === null) {
                            // FIXME; should this be a multitude of the grid-width? (see grid logic in camera.js)
                            var labelNormalizedWidth = childSliceContainer.worldContainer.containerData.label.length * 9
                            childSliceContainer.worldContainer.normalizedSize.width = labelNormalizedWidth + 10
                        }
                        childAllowedSize.width = (childSliceContainer.worldContainer.normalizedSize.width / currentNormalizedSize.width) * innerSize.width

                        childSliceContainer.opacity = childOpacity

                        my.placeSliceContainerInAllowedSize(world, childSliceContainer, childAllowedSize)

                        if (childSliceContainer.isVisible == null) {
                            if (childSliceContainer.worldContainer.containerProperties.isVisibleByDefault) {
                                childSliceContainer.isVisible = true
                            }
                            else {
                                childSliceContainer.isVisible = false
                        ...
                        // Note that the relative-width of the parent changes differently from its height (in case of a fixed header or fixed padding/margins). Essentially the inner container changes aspect ratio.
                        // We put the child at it's relative postions (X and Y separately) so it will always fit inside the parent.
                        childSliceContainer.relativePosition.x = fromParentPositionToLeftBottomOfParent.x + fromLeftBottomOfParentToBaseChildPosition.x + paddingFromLeftBottom.x + (childSliceContainer.worldContainer.normalizedPosition.x / currentNormalizedSize.width) * innerSize.width
                        childSliceContainer.relativePosition.y = fromParentPositionToLeftBottomOfParent.y + fromLeftBottomOfParentToBaseChildPosition.y + paddingFromLeftBottom.y + (childSliceContainer.worldContainer.normalizedPosition.y / currentNormalizedSize.height) * innerSize.height
                    }
                }
                else {
                    console.log("Unknown childrenFitFunction: " + childrenFitFunction)
                }
            }
        }
