...
Size2d layout_rect_within_margins(Rect2d * first_rect, FlowMargin margin)
{
    Size2d outer_size = {};
    
    first_rect->position.x = margin.horizontal;
    first_rect->position.y = margin.vertical;
    outer_size.width = margin.horizontal + first_rect->size.width + margin.horizontal;
    outer_size.height = margin.vertical + first_rect->size.height + margin.vertical;
...
void layout_elements(Flowifier * flowifier, FlowElement * flow_element)
{
    ...
    */
    ...
    else if (flow_element->type == FlowElement_Return)
    {
        ...
        {
            flow_element->rect.size = layout_rect_within_margins(&return_keyword_element->rect, flowifier->expression_margin);
        }
    ...
    else if (flow_element->type == FlowElement_For)
    {
        ...
        // We make the done-element start where the body-element started
        for_done_element->rect.position.y = for_body_element->rect.position.y;
        // We make the done-element end where the pass-right element ended, by changing its height accordingly
        for_done_element->rect.size.height = current_position_left.y - for_done_element->rect.position.y;
    ...
    else if (flow_element->type == FlowElement_Foreach)
    {
        ...
        */
        ...
        // We make the done-element start where the body-element started
        foreach_done_element->rect.position.y = foreach_body_element->rect.position.y;
        // We make the done-element end where the pass-right element ended, by changing its height accordingly
        foreach_done_element->rect.size.height = current_position_left.y - foreach_done_element->rect.position.y;
